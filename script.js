...
const quizParagraphs = [
  {
    paragraph: `NASA, the National Aeronautics and Space Administration, was established in 1958. It is responsible for the civilian space program, as well as aeronautics and aerospace research in the United States. NASA has led many space exploration missions, including the Apollo Moon landings, Mars rover missions, and the Hubble Space Telescope.`,
    questions: [
      {
        q: "In what year was NASA established?",
        options: ["1969", "1958", "1977", "1990"],
        answer: 1
      },
      {
        ...
        options: [
          ...
          "National Aeronautics and Satellite Association"
        ],
        answer: 0
      },
      {
        q: "Which country does NASA belong to?",
        options: ["Russia", "United States", "China", "India"],
        answer: 1
      },
      {
        q: "Which of these is NOT a NASA mission?",
        options: ["Apollo", "Hubble", "Voyager", "Sputnik"],
        answer: 3
      },
      {
        q: "Which planet have NASA's rovers explored?",
        options: ["Venus", "Mars", "Jupiter", "Mercury"],
        answer: 1
      },
      {
        q: "What is the Hubble?",
        options: ["A Mars rover", "A space telescope", "A Moon lander", "A rocket"],
        answer: 1
      },
      {
        q: "NASA is responsible for which type of research?",
        options: ["Aeronautics", "Aerospace", "Space", "All of the above"],
        answer: 3
      },
      {
        q: "Which NASA program landed humans on the Moon?",
        options: ["Gemini", "Apollo", "Artemis", "Mercury"],
        answer: 1
      },
      {
        q: "What is NASA's main focus?",
        options: ["Military operations", "Civilian space program", "Weather forecasting", "Telecommunications"],
        answer: 1
      },
      {
        q: "Which of these is a NASA Mars rover?",
        options: ["Curiosity", "Luna", "Venera", "Shenzhou"],
        answer: 0
      }
    ]
  },
  {
    paragraph: `The Hubble Space Telescope was launched in 1990 and has provided breathtaking images of the universe. It orbits Earth and allows astronomers to observe distant galaxies, nebulae, and stars without atmospheric distortion. Hubble has contributed to many discoveries, including the rate of expansion of the universe.`,
    questions: [
      {
      ...
      {
        q: "What does Hubble orbit?",
        options: ["The Moon", "Mars", "Earth", "The Sun"],
        answer: 2
      },
      {
        q: "What does Hubble help astronomers observe?",
        options: ["Distant galaxies", "The ocean", "Earth's core", "Weather patterns"],
        answer: 0
      },
      {
        q: "What is one advantage of Hubble being in space?",
        options: [
          "It is closer to the Sun",
          "No atmospheric distortion",
          "It can land on planets",
          "It is faster than light"
        ],
        answer: 1
      },
      {
        q: "Which of these is NOT observed by Hubble?",
        options: ["Nebulae", "Stars", "Mountains", "Galaxies"],
        answer: 2
      },
      {
        q: "What major discovery did Hubble contribute to?",
        options: [
      ...
      {
        q: "Hubble is a type of what?",
        options: ["Rover", "Telescope", "Satellite phone", "Rocket"],
        answer: 1
      },
      {
        q: "Why is Hubble important?",
        options: [
          "It provides images of the universe",
      ...
      {
        q: "Which year did Hubble start its mission?",
        options: ["1990", "2000", "1985", "2015"],
        answer: 0
      },
      {
        q: "What is a nebula?",
...
function renderQuizLevel() {
  const quizData = quizParagraphs[quizLevel % quizParagraphs.length];
  const quizParagraph = document.getElementById('quiz-paragraph');
  const questionsDiv = document.getElementById('quiz-questions');
  const quizResult = document.getElementById('quiz-result');
  const quizNextBtn = document.getElementById('quiz-next-level');
  const quizSubmit = document.getElementById('quiz-submit');

  if (!quizParagraph || !questionsDiv || !quizResult || !quizNextBtn || !quizSubmit) return;

  quizParagraph.textContent = quizData.paragraph;
  questionsDiv.innerHTML = '';
  quizData.questions.forEach((q, idx) => {
    const qDiv = document.createElement('div');
    qDiv.className = 'quiz-question';
    qDiv.innerHTML = `
      <label>Q${idx + 1}. ${q.q}</label>
...
// Setup quiz handlers after DOM is ready and after first renderQuizLevel
function setupQuizHandlers() {
  const quizForm = document.getElementById('quiz-form');
  const quizNextBtn = document.getElementById('quiz-next-level');
  const quizSubmit = document.getElementById('quiz-submit');
  if (!quizForm || !quizNextBtn || !quizSubmit) return;
  ...
  quizForm.onsubmit = function (e) {
    ...
    for (let i = 0; i < quizData.questions.length; i++) {
      ...
      if (selected === quizData.questions[i].answer) correct++;
    }
    document.getElementById('quiz-result').textContent =
      `You got ${correct} out of ${quizData.questions.length} correct!`;
  ...
  quizNextBtn.onclick = function () {
    quizLevel++;
    renderQuizLevel();
    // Re-attach handlers for new DOM
    setupQuizHandlers();
  };
}
